import fs from 'fs'
import path from 'path'
import matter from 'gray-matter'
import remark from 'remark'
import html from 'remark-html'

const playersDirectory = path.join(process.cwd(), 'players')

export function getSortedPlayersData() {
  // Get file names under /posts
  const fileNames = fs.readdirSync(playersDirectory)
  const allPlayersData = fileNames.map(fileName => {
    // Remove ".md" from file name to get id
    const id = fileName.replace(/\.md$/, '')

    // Read markdown file as string
    const fullPath = path.join(playersDirectory, fileName)
    const fileContents = fs.readFileSync(fullPath, 'utf8')

    // Use gray-matter to parse the post metadata section
    const matterResult = matter(fileContents)

    // Combine the data with the id
    return {
      id,
      ...matterResult.data
    }
  })

  // Sort posts by date
  return allPlayersData.sort((a, b) => {
    if (a.date < b.date) {
      return 1
    } else {
      return -1
    }
  })
}

export function getAllPlayerIds() {
    const fileNames = fs.readdirSync(playersDirectory)
    // fileNames returns an array of file names in posts directory as objects
    // excluding the `.md `
    
    return fileNames.map(fileName => {
        return {
            params: {
                id: fileName.replace(/\.md$/, '')
            }
        }
    })
}

// return the post data based on `id`
export async function getPlayerData(id) {
    const fullPath = path.join(playersDirectory, `${id}.md`)
    const fileContents = fs.readFileSync(fullPath, 'utf8')

    // Use gray-matter to parse the post metadata section
    const matterResult = matter(fileContents)

    // Use remark to convert markdown into html string
    const processedContent = await remark()
        .use(html)
        .process(matterResult.content)
    const contentHtml = processedContent.toString()

    // combine the data with the id
    return {
        id,
        contentHtml,
        ...matterResult.data
    }
}